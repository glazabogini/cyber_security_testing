...
import select
import errno
from AES import encrypt_message, decrypt_message  # Импорт функций из модуля AES
...
if encryption_method == "RSA":
    ...
    my_key_file = "user2_key"

    # Ключ второго пользователя
    other_key_file = "user1_key"
    ...
    #IV = bytes.fromhex(input("Enter AES IV (hex): "))
...
while True:
    ...
    if message:
        ...
        if encryption_method == "AES":
            ...
            # Перевожу в хекс, так как сервер ожидает текстовые сообщения, а из-за шифрования получает бинарные
